...
export default function HomePage() {
  return (
    <>
      ...
      <FeaturedProducts />
      <Testimonials />
      <section className="py-12 bg-manto-card border-y border-manto-border">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {trustItems.map((item) => (
              <div key={item.label} className="flex items-center gap-3 justify-center">
                ...
                <span className="text-sm font-medium text-manto-white">{item.label}</span>
              </div>
            ))}
          </div>
        </div>
      </section>
      <section className="py-20 bg-manto-neon">
        <div className="max-w-3xl mx-auto text-center px-4">
          <h2 className="text-3xl font-bold text-white mb-4">Ready to Transform Your Skin?</h2>
          <p className="text-white/60 mb-8">Join 50,000+ men who upgraded their streetwear routine with MANTO.</p>
          <a href="/products">
